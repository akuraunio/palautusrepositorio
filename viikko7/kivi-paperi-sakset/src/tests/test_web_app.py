...
class TestIndexRoute:
    ...
    def test_index_page_contains_mode_buttons(self, client):
        ...
        assert b'Tekoäly' in response.data
        assert b'Parannettu Tekoäly' in response.data
...
class TestPlayRoute:
    ...
    def test_play_page_with_session(self, client):
        ...
        assert response.status_code == 200
        assert b'Tulostaulukko' in response.data


class TestMoveRoute:
    """Tests for making moves during gameplay"""
    
    def test_valid_moves(self, client):
        ...
        # Test paper move
        response = client.post('/move', data={'move': 'p'}, follow_redirects=True)
        assert response.status_code == 200
        
        # Test scissors move
        response = client.post('/move', data={'move': 's'}, follow_redirects=True)
        assert response.status_code == 200
